...
class _YWDetailPageState extends State<YWDetailPage> {
  int likeCount = 0; // 좋아요 수를 저장하는 변수
...
class YWDetailPage extends StatefulWidget {
  ...
  final int age;
  final String imagePath;
  final Function(int) updateLikeCount; // likeCount 업데이트 함수
  ...
  const YWDetailPage({
    ...
    required this.imagePath,
    required this.age,
    required this.updateLikeCount, // 생성자에서 전달받음
  });
                      ...
                      Expanded(
                        child: ElevatedButton.icon(
                          ...
                          onPressed: () {
                            setState(() {
                              likeCount++; // 버튼을 누를 때마다 좋아요 수 증가
                              widget.updateLikeCount(likeCount); // 좋아요 수 업데이트
                            });
                          },
                          icon: const Icon(Icons.favorite, color: Colors.white),
                          label: const Text(
